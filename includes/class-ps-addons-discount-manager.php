 ...
 */
class PS_Addons_Discount_Manager {
    ...
    public static function get_discounts() {
        ...
        foreach ($results as $result) {
            array_push($discounts, new PS_Addons_Discount($result));
        }
        return $discounts;
    }

    /**
     * Apply member discounts to applicable products.
     * @param WC_Cart $cart passed from `woocommerce_cart_calculate_fees` hook.
     * @since 1.0.0
     */
    public static function apply_discount($cart) {
        if (PSCustomer()->is_member) {
