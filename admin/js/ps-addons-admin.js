(function($) {
	...
	function product_tile(product) {
		return $(`
			<div class='ps-selected-product' data-product-id="${product.id}" data-product-slug="${product.slug}" >
	...
	function status_message(style, message) {
	...
	function handle_submit(e) {
		...
		if (!name || name == '') status_message('warning', 'O campo \'Nome\' não pode ficar em branco');

		else if (!type || type == '') status_message('warning', 'O campo \'Tipo\' não pode ficar em branco');

		else if (!value || value == '') status_message('warning', 'O campo \'Valor\' não pode ficar em branco');
		
		else if (!included_products || included_products == {}) status_message('warning', 'Nenhum produto foi selecionado');

		else $.post(ps_addons_ajax.ajax_url, {
	...
	function handle_search_input() {
		...
		if ($('#product-name-input').val().length > 2) {
			$.post(ps_addons_ajax.ajax_url, {
				action: 'product_lookup',
				query: $('#product-name-input').val(),
				limit: 5
			}, null).done((res) => {
				var results = JSON.parse(res.slice(0, -1));
	...
	$('#ps-member-discounts').on('submit', handle_submit);
